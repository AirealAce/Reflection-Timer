...
(async () => {
  ...
  try {
    ...
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:8},(_,id)=>({id,available:id!==5}))}));
    check(await page.locator('#shortcut-notices kbd').allTextContents().then(keys=>keys.length===8&&keys[5]==='Ctrl+Space'&&keys[6]==='Ctrl+Alt+Space'&&keys[7]==="Ctrl+Alt+' (apostrophe)"),'Settings exposes timer and mode toggle shortcuts as styled, readable keys');
    ...
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:8},(_,id)=>({id,available:id!==6}))}));
    ...
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:8},(_,id)=>({id,available:id!==7}))}));
    ...
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:8},(_,id)=>({id,available:true}))}));
    ...
    await page.getByRole('tab',{name:'Settings',exact:true}).click();
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:[0,1,2,3,4,5,6,7].map(id=>({id,available:id!==2}))}));
    check(await page.locator('#shortcut-notices p').count()===8&&(await page.locator('#shortcut-notices p').nth(2).textContent()).includes('Unavailable'),'Settings identifies the particular unavailable global shortcut');
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:[0,1,2,3,4,5,6,7].map(id=>({id,available:true}))}));
