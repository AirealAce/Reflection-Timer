...
(async () => {
  ...
  try {
    ...
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:6},(_,id)=>({id,available:id!==5}))}));
    check(await page.locator('#shortcut-notices kbd').allTextContents().then(keys=>keys.length===6&&keys[5]==='Ctrl+Space'),'Settings exposes Ctrl+Space as a styled, readable global shortcut');
    check(await page.locator('#shortcut-notices p').last().textContent().then(text=>text.includes('any app')&&text.includes('Unavailable:'))&&await page.locator('#shortcut-notices p').first().textContent().then(text=>!text.includes('Unavailable:')),'A Ctrl+Space registration conflict is shown only beside that shortcut');
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:6},(_,id)=>({id,available:true}))}));
    check(await page.locator('#shortcut-notices').textContent().then(text=>!text.includes('Unavailable:')),'Recovered Ctrl+Space registration clears its unavailable notice');
    ...
    await page.getByRole('tab',{name:'Settings',exact:true}).click();
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:[0,1,2,3,4,5].map(id=>({id,available:id!==2}))}));
    check(await page.locator('#shortcut-notices p').count()===6&&(await page.locator('#shortcut-notices p').nth(2).textContent()).includes('Unavailable'),'Settings identifies the particular unavailable global shortcut');
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:[0,1,2,3,4,5].map(id=>({id,available:true}))}));
