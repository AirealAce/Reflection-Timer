...
(async () => {
  ...
  try {
    ...
    const deliveryIssue={errorKind:'network',message:'The network request failed. Reflections remain saved. Use Send pending now to retry sooner.',nextRetryAt:Date.UTC(2026,8,22,12,15)};
    await page.evaluate(({state,issue})=>{window.previewDispatch({type:'state',state});window.previewDispatch({type:'deliveryIssue',issue});},{state:{...changed,connected:true},issue:deliveryIssue});
    check(await page.locator('#delivery-status').isVisible()&&!await page.locator('#delivery-status').evaluate(el=>el.classList.contains('sr-only')),'Connection failure status is visible in the Outbox, not restricted to screen readers');
    check(await page.locator('#delivery-status').textContent().then(text=>text.includes('network request failed')&&text.includes('Automatic retry after')&&text.includes('Send pending now')),'Outbox explains failed automatic connection checks and their next retry');
    check(await page.locator('#delivery-status').evaluate(el=>!el.closest('[aria-live],[role="status"],[role="alert"]')),'Background retry status does not become a repeating live-region announcement');
    await page.locator('#send-pending').focus();
    await page.evaluate(state=>window.previewDispatch({type:'state',state}),{...changed,connected:true});
    check(await page.locator('#delivery-status').textContent().then(text=>text.includes('network request failed'))&&await page.locator('#send-pending').evaluate(el=>el===document.activeElement),'Timer state updates retain the delivery issue without stealing focus');
    await page.locator('#send-pending').click();
    check(await page.evaluate(()=>window.previewMessages.some(m=>m.action==='sendPending')),'Send pending now remains available while a connection retry is waiting');
    await page.evaluate(settings=>window.previewDispatch({type:'settings',settings}),{...settings,connected:true,deliveryIssue});
    check(await page.locator('#delivery-status').textContent().then(text=>text.includes('Automatic retry after')),'Loading settings restores an existing connection issue for a reopened app window');
    await page.evaluate(()=>window.previewDispatch({type:'deliveryIssue',issue:{errorKind:'rejected',message:'Review Connection setup, then use Save & test connection or Send pending now.',nextRetryAt:null}}));
    check(await page.locator('#delivery-status').textContent().then(text=>text.includes('Connection setup')&&!text.includes('Automatic retry')),'Access failures show corrective actions without promising an automatic retry');
    await page.evaluate(()=>window.previewDispatch({type:'deliveryIssue',issue:{message:'<img src=x onerror=alert(1)>',nextRetryAt:null}}));
    check(await page.locator('#delivery-status img').count()===0&&await page.locator('#delivery-status').textContent()==='<img src=x onerror=alert(1)>','Delivery status renders as text, never executable markup');
    await page.evaluate(()=>window.previewDispatch({type:'deliveryIssue',issue:null}));
    check(await page.locator('#delivery-status').textContent().then(text=>text.includes('enabled')&&!text.includes('Automatic retry')),'Successful verification clears the failure message and retry time');
    await page.evaluate(state=>window.previewDispatch({type:'state',state}),changed);
    ...
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:9},(_,id)=>({id,available:id!==5}))}));
    check(await page.locator('#shortcut-notices kbd').allTextContents().then(keys=>keys.length===9&&keys[5]==='Ctrl+Space'&&keys[6]==='Ctrl+Alt+Space'&&keys[7]==="Ctrl+Alt+' (apostrophe)"&&keys[8]==='Ctrl+Alt+R'),'Settings exposes timer, mode toggle, and reset shortcuts as styled, readable keys');
    ...
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:9},(_,id)=>({id,available:id!==6}))}));
    ...
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:9},(_,id)=>({id,available:id!==7}))}));
    ...
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:9},(_,id)=>({id,available:true}))}));
    ...
    const eventEditor=page.getByRole('group',{name:'Session end · timer / stopwatch',exact:true});
    ...
    await page.getByRole('spinbutton',{name:'Low-time threshold in seconds',exact:true}).fill('18');
    await page.keyboard.press('Control+Enter');
    await page.waitForFunction(()=>window.previewMessages.some(m=>m.action==='saveAppearance'));
    check(await page.evaluate(()=>window.previewMessages.findLast(m=>m.action==='lowTime').data.threshold)===18,'Save settings keyboard shortcut saves the shared Timer threshold');
    ...
    for(const [name,key] of [['Success messages','Success'],['Failure messages','Failure'],['Low on time audio','LowTime'],['Time reached · stopwatch','TimeReached'],['Session end · timer / stopwatch','SessionEnd']]){
    ...
    await page.getByRole('tab',{name:'Settings',exact:true}).click();
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:9},(_,id)=>({id,available:id!==2}))}));
    check(await page.locator('#shortcut-notices p').count()===9&&(await page.locator('#shortcut-notices p').nth(2).textContent()).includes('Unavailable'),'Settings identifies the particular unavailable global shortcut');
    await page.evaluate(()=>window.previewDispatch({type:'shortcuts',shortcuts:Array.from({length:9},(_,id)=>({id,available:true}))}));
    ...
    check(await page.getByRole('spinbutton').count()===3&&await page.getByRole('button').count()===10&&await page.getByRole('button',{name:'Auto-start next session',pressed:false,exact:true}).count()===1,'Compact exposes its original actions, mode switch, and accessible Auto-start toggle button');
    ...
    await require('./checkbox-fields.cjs')(context,initial,settings,check);
    await require('./timer-enter.cjs')(context,initial,check);
    await require('./app-playback.cjs')(context,initial,check);
    await require('./time-only-toggle.cjs')(context,initial,check);
