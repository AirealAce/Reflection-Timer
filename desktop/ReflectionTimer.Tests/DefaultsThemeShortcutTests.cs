        ...
        var thread=new Thread(()=>{
            try {
                ...
                check(GlobalShortcut.MessageDelay(100,350)==TimeSpan.FromMilliseconds(250)
                    &&GlobalShortcut.MessageDelay(int.MaxValue-10,int.MinValue+9)==TimeSpan.FromMilliseconds(20)
                    &&GlobalShortcut.MessageDelay(-10,10)==TimeSpan.FromMilliseconds(20)
                    &&GlobalShortcut.MessageDelay(350,100)==TimeSpan.Zero,"Windows message delay handles tick wraparound and never moves a stop into the future");
