...
module.exports=async function reflectionSubmit(context,initial,check){
  ...
  const submitted=(page,action='saveOrSendReflection')=>page.waitForFunction(action=>window.previewMessages.some(m=>m.action===action),action);
  ...
  for(const kind of ['natural','early','check-in']){
    ...
    for(const target of targets)for(const key of ['Control+Enter','Alt+Enter']){
      ...
      const action=key==='Control+Enter'?'saveOrSendReflection':'queue';
      ...
      check(entry.length===1&&entry[0].data.id==='submit-test'&&entry[0].data.text==='Saved response'&&entry[0].data.reason==='Saved reason'&&entry[0].data.endSession===undefined,`${kind} ${key} delegates the current fields to ${action} without an early-ending request from ${target}`);
      check(!sent.some(m=>['saveForLater','skip','navigateReflection','toggle','end','checkIn',action==='queue'?'saveOrSendReflection':'queue'].includes(m.action)),`${kind} ${key} overrides the focused action without a separate unscoped timer command`);
      await page.close();
    }
  }
  ...
  await page.evaluate(()=>{
    ...
    window.chrome.webview.postMessage=message=>{
      if(!['draft','saveOrSendReflection'].includes(message.action))return normal(message);
  ...
  check((await messages(page)).filter(m=>m.action==='draft').length===1&&!(await messages(page)).some(m=>['queue','saveOrSendReflection'].includes(m.action))&&await page.locator('#reflection-text').evaluate(e=>e.readOnly),'Repeated Ctrl+Enter waits for one durable draft save and freezes edits');
  ...
  check(await page.locator('#reflection-text').isEditable()&&await page.locator('#reflection-text').inputValue()==='Final keystrokes'&&!(await messages(page)).some(m=>['queue','saveOrSendReflection'].includes(m.action)),'Failed draft save restores editing and retains the response without sending');
  ...
  await page.keyboard.press('Control+Enter');
  check((await messages(page)).filter(m=>m.action==='saveOrSendReflection').length===1,'Repeated Ctrl+Enter cannot duplicate a pending save/send');
  ...
  await page.keyboard.press('Control+Enter');
  await page.waitForFunction(()=>window.previewMessages.filter(m=>m.action==='saveOrSendReflection').length===2);
  const sent=await messages(page),retry=sent.filter(m=>m.action==='saveOrSendReflection')[1];
  check(retry.data.text==='Final keystrokes'&&retry.data.reason==='Final reason'&&retry.data.endSession===undefined&&sent.filter(m=>m.action==='draft').length===2,'Retry rechecks the corresponding session using the latest durable fields without an early-ending request');
  await page.evaluate(()=>window.previewDispatch({type:'reply',requestId:window.queueRequest.requestId}));
  await page.keyboard.press('Control+Enter');
  check((await messages(page)).filter(m=>m.action==='saveOrSendReflection').length===2,'Successful save/send remains guarded until the native window closes or loads another reflection');
  await page.close();
  ...
  check((await messages(clicked)).find(m=>m.action==='queue').data.endSession===undefined,'The Save & send button keeps its existing non-ending check-in behavior');
  await clicked.close();
  ...
  await blank.evaluate(()=>{
    const normal=window.chrome.webview.postMessage;
    window.chrome.webview.postMessage=message=>{
      if(message.action!=='saveOrSendReflection')return normal(message);
  ...
  check(!(await messages(guarded)).some(m=>['queue','saveOrSendReflection'].includes(m.action)),'Other modifier combinations and composition cannot accidentally save or send a session');
  ...
  check(!(await messages(guarded)).some(m=>['queue','saveOrSendReflection'].includes(m.action)),'An open modal retains Ctrl+Enter instead of submitting the reflection behind it');
  ...
  check(!(await messages(guarded)).some(m=>['queue','saveOrSendReflection'].includes(m.action)),'Window-wide Ctrl+Enter respects automatic replacement and navigation freezes');
  await guarded.close();
  for(const [view,initialize] of [['main',true],['compact',true],['reflection',false]]){
    ...
    check(!(await messages(other)).some(m=>['queue','saveOrSendReflection'].includes(m.action)),`Ctrl+Enter does not save/send from ${initialize?view:'an uninitialized reflection'}`);
    await other.close();
  }
};
