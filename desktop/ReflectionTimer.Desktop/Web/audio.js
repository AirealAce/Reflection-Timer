...
export function mountAudio({send,run}) {
  ...
  for(const kind of [1,2,3,4,0]){
    ...
    form.addEventListener('input',event=>{if(event.target.closest('#settings-low-options,.time-reached-options'))return;editor.dirty=true;editor.revision++;if(event.target===field('sound-volume')){setText(field('sound-volume-caption'),`Volume (${event.target.value}%)`);run(saveSound);}});
  ...
  function renderEditor(editor){if(!settings||editor.dirty)return;const sound=settings.sounds.find(s=>s.kind===editor.kind)??(editor.kind===4?settings.sounds.find(s=>s.kind===3):undefined),field=editor.field;
    if(!sound)return;
