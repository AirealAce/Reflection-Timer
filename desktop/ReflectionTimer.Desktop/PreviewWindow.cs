...
internal sealed partial class PreviewWindow : Form, IReflectionPromptWindow, IReflectionShortcutTarget
{
    ...
    internal PreviewWindow(PreviewApplication app, string view, Guid? prompt)
    {
        ...
        Text = view == "main" ? "Reflection Timer — App view · 4.2.10" : view == "compact" ? "Reflection Timer — Compact view · 4.2.10" : "Reflection Timer — Session end · 4.2.10";
    ...
    private async void Receive(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        ...
        try {
            ...
            if (action == "settingsShortcutScope") {
                if(View!="main")throw new ArgumentException("Settings shortcuts belong to App view.");
                settingsShortcutsActive=ReadFlag(data,"enabled");Reply(requestId);return;
            }
            if (action == "resetAndReload") { await ResetAndReloadAsync(requestId); return; }
            if (action == "reset") { await ResetFromButtonAsync(data,requestId,requestedAt); return; }
            ...
            if (action == "compactSize") {
                ...
                ApplyTopMost();
                Text="Reflection Timer — "+(IsTimeOnly?"Time-only":"Compact")+" view · 4.2.10";
